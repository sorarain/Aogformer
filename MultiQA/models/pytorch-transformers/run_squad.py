...
from pytorch_transformers import AutoConfig, RobertaTokenizer
from torch.nn import CrossEntropyLoss
import torch.nn as nn
from pytorch_transformers.modeling_bert import BertPreTrainedModel
from AOGformer import RobertaAOGForMaskedLM
...
class RobertaLongSelfAttention(LongformerSelfAttention):
    def forward(
        ...
        return super().forward(hidden_states, attention_mask=attention_mask.squeeze(1).squeeze(1), output_attentions=output_attentions)
...
class LongformerForQuestionAnswering(BertPreTrainedModel):
    ...
    def __init__(self, config, model):
        super(LongformerForQuestionAnswering, self).__init__(config)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None,
        ...
        outputs = self.bert(input_ids, position_ids=position_ids, token_type_ids=token_type_ids,
                            attention_mask=attention_mask, head_mask=head_mask, output_hidden_states=True)
...
MODEL_CLASSES = {
    ...
    'longformer': (AutoConfig,RobertaLongForMaskedLM, RobertaTokenizerFast),
    'aogformer': (AutoConfig,RobertaAOGForMaskedLM, RobertaTokenizerFast)
}
    ...
    for epoch_number in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {'input_ids':       batch[0],
                      ...
                      'token_type_ids':  None if args.model_type == 'xlm' or args.model_type in ['longformer', 'aogformer'] else batch[2],
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            inputs = {'input_ids':      batch[0],
                      'attention_mask': batch[1],
                      'token_type_ids': None if args.model_type == 'xlm' or args.model_type in ['longformer', 'aogformer'] else batch[2]  # XLM don't use segment_ids
                      }
...
def main():
    ...
    model = model_class.from_pretrained(args.model_name_or_path) if args.model_type in ['longformer', 'aogformer'] else model_class.from_pretrained(args.model_name_or_path, from_tf=bool('.ckpt' in args.model_name_or_path), config=config)
    model = LongformerForQuestionAnswering(config, model) if args.model_type in ['longformer', 'aogformer'] else model
    config = model.config if args.model_type in ['longformer', 'aogformer'] else config
    tokenizer = RobertaTokenizerFast.from_pretrained(args.tokenizer_name) if args.model_type in ['longformer', 'aogformer'] else tokenizer
