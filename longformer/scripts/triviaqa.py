...
class TriviaQA(pl.LightningModule):
    ...
    def load_model(self):
        if 'longformer' in self.args.model_path and not 'aog' in self.args.model_path and not 'fam' in self.args.model_path:
            ...
            if self.args.seq2seq:
                ...
                model = AutoModel.from_pretrained(self.args.model_path, config=config)
        elif 'aog' in self.args.model_path:
            model = RobertaAOGForMaskedLM.from_pretrained(self.args.model_path)
            self.args.attention_window = 512
        elif 'fam' in self.args.model_path:
            model = RobertaFAMForMaskedLM.from_pretrained(self.args.model_path)
        else:
    ...
    def forward(self, input_ids, attention_mask, segment_ids, start_positions, end_positions, answer_token_ids):
        if 'longformer' in self.args.model_path or 'aog' in self.args.model_path or 'fam' in self.args.model_path:
            ...
            if 'fam' in self.args.model_path:
                ...
                sequence_output = self.model(
                        input_ids,
                        attention_mask=attention_mask)
                if 'aog' in self.args.model_path:
